...
namespace eZ\Publish\Core\Persistence\SqlNg\Content\Search\Gateway\SortClauseHandler;

use eZ\Publish\Core\Persistence\SqlNg\Content\Search\Gateway\SortClauseHandler;
    ...
    public function applyJoin( ezcQuerySelect $query, SortClause $sortClause, $number )
    {
        ...
        $query
            ->leftJoin(
                ...
                $query->expr->eq(
                    $this->dbHandler->quoteColumn( 'section_id', $table ),
                    $this->dbHandler->quoteColumn( 'section_id', 'ezcontent' )
                )
            );
    }
}
